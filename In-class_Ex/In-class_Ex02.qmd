...
```{r}
geoboundaries26391 <- st_set_crs(geoboundaries, 26391)
```

```{r}
st_crs(geoboundaries26391)
```

#Importing the aspatial data

```{r}
waterpoint <-read_csv(("Data/Aspatial/Water_Point_Data_Exchange.csv"))
```
...
```{r}
waterpoint_sf <- st_as_sf(waterpoint, 
