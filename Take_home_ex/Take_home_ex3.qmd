...
## 7.2 Building the OLS Model using olsrr
...
```
Based on the results above, we can see that the P-Value for all are not statistically significant, meaning that it’s p-value is more than or equal to 0.05. Therefore, we are unable to statistically conclude that the predictor is significant in contributing towards the model.

## 7.3 Testing Linear Regression 

### 7.3.1 Multicolinearity
...
## 7.4 Geographically Weighted Regression Predictive Method
...
## 7.5 Preparing Coordinates Data
...
## 7.6 Calibrating Random Forest Model
...
### 7.7.1 Calibrating using training data.
...
### 7.7.2 Predicting using Test Data
...
### 7.7.3 Calculating Root Mean Square Error
...
The OLS Multiple Linear Regression has a RMSE error of 48428.6 which depicts the mean error for the HDB resale prices prediction is +/- 48428.6. For Geographically Weighted Random Forest, the mean error for HDB resale prices prediction is +/- 32313.2.

As we can see, OLS Multiple Linear Regression has a higher RMSE score than Geographically Weighted Random Forest. Therefore, the Geographically Weighted Random Forest is the better model.

### 7.7.4 Visualising the predicted values
