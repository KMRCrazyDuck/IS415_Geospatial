...
## 4.3 Splitting of training and testing dataset

```{r}
rs_coords_sf_training <-  filter(rs_coords_sf ,flat_type == "4 ROOM") %>% 
              filter(month >= "2021-01" & month <= "2022-12")
```

```{r}
rs_coords_sf_testing <-  filter(rs_coords_sf ,flat_type == "4 ROOM") %>% 
...
rs_factors_rds_testing <- write_rds(rs_coords_sf_testing, "Data/rds/rs_factors_testing.rds")
```
...
rs_factors_testing <- read_rds("Data/rds/rs_factors_testing.rds")
```
...
rs_factors_testing <- left_join(rs_factors_testing, storey_range_order, by= c("storey_range" = "storeys"))
```
...
rs_factors_testing <- rs_factors_testing %>%
  select(resale_price, floor_area_sqm, storey_order, remaining_lease_mths,
         PROX_CBD, PROX_ELDERLYCARE, PROX_HAWKER, PROX_MRT, PROX_PARK, PROX_GOOD_PRISCH, PROX_MALL, PROX_CHAS,
         PROX_SUPERMARKET, WITHIN_350M_KINDERGARTEN, WITHIN_350M_CHILDCARE, WITHIN_350M_BUS, WITHIN_1KM_PRISCH)
```
...
## 6.1 HDB 4 room resale prices in Histogram
...
## 6.2 Normalise using Log Transformation of HDB 4 room resale prices in Histogram
...
PROX_GOOD_PRISCH have some patterns involved whereby there more transactions happening within the proximity of approximately 3km and 6km. PROX_CBD have a left skewed distribution, this shows majority of the transactions HDB resale flats are away from CBD.
...
From the interactive map, we can see that 4 room resale HDBs around the South and Central tend to have higher resale prices which is indicated by the darker orange points. This is in comparison to the lighter yellow points concentrated around the North and East area.
...
```{r}
set.seed(1234)
# Training Data a total of 26 months, 24 months training, 2 months testing
# 2/26 will be 0.0769, 10 - 0.077 = 9.22
resale_split <- initial_split(rs_coords_sf, 
                              prop = 9.22/10,)
...
The correlation matrix above shows that all the correlation values are below 0.8 with the exception of WITHIN_350M_CHILDCARE that has a value of 0.94. This shows that WITHIN_350M_CHILDCARE has a sign of multicolinearity with WTIHIN_350M_KINDERGARTEN
...
The result shows that 114 neighbour points will be the optimal bandwidth to be used if adaptive bandwidth is used for this data set.
