...
![](/_site/Take_home_ex/Take_home_ex1_files/figure-html/unnamed-chunk-53-1.png)
...
![](/_site/Take_home_ex/Take_home_ex1_files/figure-html/unnamed-chunk-55-1.png)
...
## 7.2 Applying Local Colocation Quotients

```{r}
nb <- include_self(
  st_knn(st_geometry(wpdx_sf_nga), 6))
```
...
```{r}
LCLQ <- local_colocation(A, B, nb, wt , 30)
```

```{r}
LCLQ_sf_nga <- cbind(wpdx_sf_nga, LCLQ)
```
